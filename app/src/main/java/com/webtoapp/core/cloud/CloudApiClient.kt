...
class CloudApiClient(private val tokenManager: TokenManager, context: android.content.Context? = null) {
    ...
    /** 获取内置标签列表 */
    suspend fun getCommunityTags(): AuthResult<List<String>> = authRequest { token ->
        val request = Request.Builder().url("$BASE_URL/api/v1/community/tags").header("Authorization", "Bearer $token").get().build()
        val response = client.newCall(request).execute()
        val responseBody = response.body?.string() ?: ""
        if (response.isSuccessful) {
            val json = JsonParser.parseString(responseBody).asJsonObject
            val data = json.getAsJsonArray("data")
            AuthResult.Success(data?.map { it.asString } ?: emptyList())
        } else AuthResult.Error(parseError(responseBody, response.code))
    }
